...
while !happy
    ...
    new_pw = pw_options.sample #pick one of those combos
    #print new_pw
    new_pw.map! do |x| x.capitalize #capitalise the first letter of each element chosen
    end
    joined_pw = new_pw.join #take each element and make it one long string
    random_number = num.rand(100..999).to_s #generate a random number from 0-999 and make it a string
    ...
    while do_you_like
        ...
        if response == "y" #program ends
            ...
            puts "I didn't get that" #ask again if they are happy
        end
    end
end
